...
Reading, drawing and local progress do not need an AI connection. If you configure your own Gemini API key and use Check work, AI solutions or chat, the app sends the relevant question or textbook excerpts, your message, selected handwritten work/images, and relevant conversation or grading context directly to Google's Gemini API over HTTPS. A connection test sends a short test prompt. Google also receives connection metadata such as your IP address. The developer does not operate an intermediary grading server.

Google processes requests under the Gemini API terms and privacy policies. This app requires a project with active billing for AI use; Google requires Paid Services for API clients offered in Switzerland, the EEA and the UK. Retention, human review and use to improve models depend on service terms, billing status and location. The app cannot verify your billing status or delete copies retained by Google. Consult https://ai.google.dev/gemini-api/terms and https://policies.google.com/privacy before using AI. Google's eligibility, age and regional restrictions apply. AI can make mistakes and is not a formal assessment authority.

## Adult eligibility
Before entering the app, you are asked to confirm that you are 18 or older. Before making AI requests, you must also confirm the provider's age, region and active-billing requirements. These declarations are stored locally. The app does not collect a birth date or identity document and does not claim to verify your age. It is not intended for children or other users under 18.
...
The saved API key is encrypted using Android Keystore, tied to your device, and excluded from the app's ZIP backup and configured Android backup rules. You can remove it in Settings. Request counts stay on the device and are not a provider quota or billing measurement.
...
Use the app's profile and data-management controls to remove local work, or clear Android app storage/uninstall to remove the app's private data. These actions do not delete ZIPs you saved to Downloads, shared copies, email correspondence, system backups, submitted AI reports or data held by Google. Delete those separately through their respective services. Keep a separate backup of important work; no device or software can guarantee permanent storage.

## Permissions
Internet access supports optional direct Google AI requests and reports you choose to submit to the developer. Network-state access supports connection handling. Backup files use Android's scoped storage or temporary content-URI access; the app does not request broad access to all your files, location, contacts, microphone or camera.
